...
def get_musdbhq(database_path):
    ...
    for subset in ["train", "test"]:
        ...
        for track_folder in sorted(tracks):
            ...
            example = dict()
            for stem in ["Mixed", "Cough", "Speech"]:
                filename = stem
    ...
    for subset in ["train", "test"]:
        ...
        for track in sorted(tracks):
            ...
            if os.path.exists(mix_path):
                ...
                paths.update({key : track_path + "_" + key + ".wav" for key in ["Speech", "Cough"]})
            ...
            # Add other instruments to form accompaniment
            acc_audio = np.clip(sum([stem_audio[key] for key in list(stem_audio.keys()) if key != "Speech"]), -1.0, 1.0)
            ...
            diff_signal = np.abs(mix_audio - acc_audio - stem_audio["Speech"])
            print("Maximum absolute deviation from source additivity constraint: " + str(np.max(diff_signal)))# Check if acc+Speech=mix
...
def get_musdb_folds(root_path, version="HQ"):
    ...
    train_list = np.random.choice(train_val_list, 1600, replace=False)
